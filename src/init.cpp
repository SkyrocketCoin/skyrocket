...
void PrepareShutdown()
{
    ...
    RenameThread("skyrocket-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "skyrocket.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "skyrocketd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, skyrocket, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-anonymizeskyrocketamount=<n>", strprintf(_("Keep N BWK anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
	       ...
	       "\n" +
	       FormatParagraph(strprintf(_("Copyright (C) 2017-%i The SkyRocket Core Developers"), COPYRIGHT_YEAR)) + "\n" +
	       "\n" +
...
{
    RenameThread("skyrocket-loadblk");
...
/** Sanity checks
 *  Ensure that Skyrocket is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Skyrocket Core is shutting down."));
...
#endif
    // Make sure only a single Skyrocket process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Skyrocket Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Skyrocket version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Skyrocket Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Skyrocket Core to complete") << "\n";
    ...
    nAnonymizeSkyrocketAmount = GetArg("-anonymizeskyrocketamount", 0);
    if (nAnonymizeSkyrocketAmount > 999999) nAnonymizeSkyrocketAmount = 999999;
    if (nAnonymizeSkyrocketAmount < 2) nAnonymizeSkyrocketAmount = 2;
    ...
    LogPrintf("Anonymize Skyrocket Amount %d\n", nAnonymizeSkyrocketAmount);
