...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SkyRocketMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SkyRocketMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SkyRocketMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("skyrocket-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SkyRocketMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
