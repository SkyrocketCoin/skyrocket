...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Skyrocket address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Skyrocket: URI
...
{
    // Convert skyrocket:// to skyrocket:
    //
    //    Cannot handle this later, because skyrocket:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open skyrocket.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Skyrocket.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Skyrocket.lnk
...
{
    return GetAutostartDir() / "skyrocket.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a skyrocket.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Skyrocket\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the skyrocket app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add skyrocket app to startup item list
