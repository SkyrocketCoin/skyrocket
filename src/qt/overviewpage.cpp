...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSkyrocketAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSkyrocketAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSkyrocketAmount = strAnonymizeSkyrocketAmount.remove(strAnonymizeSkyrocketAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkyrocketAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSkyrocketAmount * COIN) nMaxToAnonymize = nAnonymizeSkyrocketAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSkyrocketAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSkyrocketAmount));
        strAnonymizeSkyrocketAmount = strAnonymizeSkyrocketAmount.remove(strAnonymizeSkyrocketAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkyrocketAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeSkyrocketAmount == 0) {
